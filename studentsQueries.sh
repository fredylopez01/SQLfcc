...
echo "$($PSQL "SELECT course FROM students FULL JOIN majors USING(major_id) FULL JOIN majors_courses USING(major_id) FULL JOIN courses USING(course_id) WHERE students.student_id IS NOT NULL GROUP BY courses.course HAVING COUNT(*) = 1 ORDER BY course;")"